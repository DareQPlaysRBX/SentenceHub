...
assert(Window, "[ SENTENCE ] Universal: _G.Window is nil — did Loader set it?")
assert(Lib,    "[ SENTENCE ] Universal: _G.Lib    is nil — did Loader set it?")
...
task.defer(function()
    for _, obj in ipairs(LP.PlayerGui:GetChildren()) do
        if obj:IsA("ScreenGui") then
            pcall(function() obj.Parent = _GUI_PARENT end)
        end
    end
end)

-- ════════════════════════════════════════════════════════════
-- SHARED STATE  (all features off by default)
-- ════════════════════════════════════════════════════════════
local UNI = {
    FlyEnabled       = false,
    NoclipEnabled    = false,
...
local function startAntiFling()
    ...
    AntiFlingConn = RunService.Heartbeat:Connect(function()
        ...
        if not root then _lastSafeCF = nil; return end
...
-- ════════════════════════════════════════════════════════════
-- MAIN TAB  —  Movement & Protection & Utility
-- ════════════════════════════════════════════════════════════
...
-- ── Cleanup ────────────────────────────────────────────────
local ESP_BIND        = "SentenceHub_Universal_ESP_v23"
...
RunService:BindToRenderStep(ESP_BIND, Enum.RenderPriority.Camera.Value + 1, function()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if CFG.ShowHealthBar then
            ...
            set(d.hpBarBg, { Visible=true, Filled=true, Position=V2(bx_-1,topSP.Y-1), Size=V2(bw+2,boxH+2), Color=DARK_BG, Transparency=0.5 })
            set(d.hpBar,   { Visible=true, Filled=true, Position=V2(bx_,topSP.Y+boxH*(1-pct)), Size=V2(bw,math.max(boxH*pct,1)), Color=hpCol, Transparency=1 })
        else
            d.hpBar.Visible=false; d.hpBarBg.Visible=false
        end
...
S1:CreateToggle({
    ...
    Callback = function(v)
        CFG.ShowChams = v
        if not v then
            for p in pairs(AllHighlights) do destroyHighlight(p) end
        end
    end,
})
