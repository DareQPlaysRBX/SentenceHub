...
local function buildSectionAPI(page, accent, secondary)
    ...
    function API:CreateSection(name)
        ...
        function Sec:CreateLabel(cfg)
            ...
            local card, strip, stroke = makeElemFrame(container, 30)
            if cfg.Style > 1 then
        ...
        function Sec:CreateButton(cfg)
            ...
            local h = cfg.Description and 50 or 34
            ...
            newText({
                ...
                Position = UDim2.new(0,12,0, cfg.Description and 7 or 9),
                Font     = Enum.Font.GothamBold,
                TextSize = 13,
                Color    = T.TextHi,
                Z        = 6,
        ...
        function Sec:CreateToggle(cfg)
            ...
            local function refresh()
                if TV.CurrentValue then
                    ...
                    tw(knob,       {Position=UDim2.new(1,-16,0.5,0), BackgroundColor3=accent,BackgroundTransparency=0}, TI_ELASTIC)
                    ...
                    task.spawn(function()
                        tw(knob,{Size=UDim2.new(0,16,0,16)},TI(.12,Enum.EasingStyle.Back))
                        task.wait(0.13)
                        tw(knob,{Size=UDim2.new(0,13,0,13)},TI_FAST)
                    end)
                else
        ...
        function Sec:CreateSlider(cfg)
            ...
            local card, strip, stroke = makeElemFrame(container, 50)
            strip.BackgroundTransparency = 0
        ...
        function Sec:CreateInput(cfg)
            ...
            local h = cfg.Description and 64 or 50
            local card, strip, stroke = makeElemFrame(container, h)
            strip.BackgroundTransparency = 0
            ...
            local fieldY = cfg.Description and 35 or 22
            local fieldH = 19

            local fieldBg = newFrame({
                Size     = UDim2.new(1,-24,0,fieldH),
                Position = UDim2.new(0,12,0,fieldY),
                Color    = T.BG1,
                Alpha    = 0,
                Radius   = 3,
                Z        = 5,
        ...
        function Sec:CreateDropdown(cfg)
            ...
            local baseH = cfg.Description and 64 or 50
            ...
            local header = newFrame({
                Size     = UDim2.new(1,-24,0,headerH),
                Position = UDim2.new(0,12,0,headerY),
                Color    = T.BG1,
                Alpha    = 0,
                Radius   = 4,
                Z        = 5,
...
function Frost:Notify(data)
    task.spawn(function()
        ...
        -- Push existing notifs down
        local existing = self._notifHolder:GetChildren()
        for _, child in ipairs(existing) do
            if child:IsA("Frame") then
                tw(child, {Position=UDim2.new(0,0,0,child.Position.Y.Offset+68)}, TI_SPRING)
                tw(child, {Size=UDim2.new(0,270,0,child.Size.Y.Offset * 0.96),
                            BackgroundTransparency=child.BackgroundTransparency+0.04}, TI_MED)
            end
        end
        ...
        local cardStroke = newStroke(card, pal.stroke, 1, 0.72)
        ...
        newGradient(sideGlow, nil, NumberSequence.new{
        ...
        local iconRingStroke = newStroke(iconRing, pal.fg, 1, 1)
        ...
        closeBtn.MouseEnter:Connect(function()
            tw(closeBtn,  {BackgroundColor3=T.Error},   TI_FAST)
            tw(closeLabel,{TextColor3=T.TextHi},         TI_FAST)
        end)
        closeBtn.MouseLeave:Connect(function()
            tw(closeBtn,  {BackgroundColor3=T.BG3},     TI_FAST)
            tw(closeLabel,{TextColor3=T.TextLo},         TI_FAST)
        end)

        -- Wait for layout, get real height
        task.wait()
        local cardH = content.AbsoluteSize.Y + 4
        iconRing.Position = UDim2.new(0,10,0, cardH/2 - 11)
        card.Size = UDim2.new(0,270,0, cardH)
        ...
        -- ── Entrance: drop from top (spring) ──────────────────────────────────
        tw(card, {Position=UDim2.new(0,0,0,4)}, TI_SPRING)
        task.wait(0.09)

        local TI_IN = TI(.18, Enum.EasingStyle.Exponential)
        tw(shimmer,   {BackgroundTransparency=0.94},  TI_IN)
        tw(sideGlow,  {BackgroundTransparency=0.88},  TI_IN)
        tw(iconRing,  {BackgroundTransparency=0},     TI_IN)
        tw(iconRingStroke,{Transparency=0.30},         TI_IN)
        tw(cardStroke,{Transparency=0.45},             TI_IN)
        ...
        tw(timerBg,   {BackgroundTransparency=0.6},   TI_IN)
        ...
        card.MouseEnter:Connect(function()
            paused = true
            tw(card, {Size=UDim2.new(0,270, 0, cardH * 1.02)}, TI_FAST)
            tw(cardStroke,{Transparency=0.22}, TI_FAST)
        end)
        card.MouseLeave:Connect(function()
            paused = false
            tw(card, {Size=UDim2.new(0,270, 0, cardH)}, TI_FAST)
            tw(cardStroke,{Transparency=0.45}, TI_FAST)
        end)
        ...
        -- Slide right + shrink
        tw(card, {
            Position = UDim2.new(0, 30, 0, card.Position.Y.Offset),
            BackgroundTransparency = 1,
            Size = UDim2.new(0,270,0,0),
        }, TI(.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
...
function Frost:CreateWindow(cfg)
    ...
    local WW   = math.clamp(vp.X - 100, 540, 760)
    local WH   = math.clamp(vp.Y -  80, 390, 510)
    local FULL = UDim2.fromOffset(WW, WH)
    local TBH  = 38   -- title bar height
    local SBW  = 46   -- sidebar width
    ...
    newLayout(notifHolder, 5, Enum.FillDirection.Vertical)
    ...
    logoI.Name  = "WinLogo"
    logoI.Size  = UDim2.new(0,22,0,22)
    logoI.Position  = UDim2.new(0,12,0.5,0)
    ...
    Instance.new("UICorner",logoI).CornerRadius = UDim.new(0,4)
    ...
    local winName = newText({
        ...
        Position = UDim2.new(0,txOff,0,4),
        Font     = Enum.Font.GothamBold,
        TextSize = 14,
        Color    = T.TextHi,
        Alpha    = 1,
    ...
    local winSub = newText({
        ...
        Position = UDim2.new(0,txOff,0,21),
        Font     = Enum.Font.Code,
        TextSize = 10,
        Color    = T.TextLo,
        Alpha    = 1,
